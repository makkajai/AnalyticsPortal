...
namespace AnalyticsPortal.Web.Helpers
{
    public class InternalDashboardData
    {
        ...
        @"select max(date) as LastPlayedDate, min(date) as FirstPlayedDate, case when count(1) > 1 then count(1) else 0 end as LogCount, s.login as Login, FirstName, LastName
